...
describe("Merkle tree", function () {
    ...
    it("works on-chain too", async function () {
        ...
        const { clothes } = await getContract({merkleRoot:root})
        expect(await clothes.balanceOf(signer.address)).to.equal(0)
        await expect(
            clothes.mint(proof, {value: ethers.utils.parseEther("0.2")})
        ).to.be.revertedWith('wrong payment');
        await expect(
            clothes.mint(proof)
        ).to.be.revertedWith('wrong payment');
        await clothes.mint(proof, {
            value: ethers.utils.parseEther("0.1")
        });
        expect(await clothes.balanceOf(signer.address)).to.equal(1)
        await expect(
            clothes.mint(proof, {
                value: ethers.utils.parseEther("0.1")
            })
        ...
        await expect(
            clothes.connect(attacker).mint(proof, {value: ethers.utils.parseEther("0.1")})
