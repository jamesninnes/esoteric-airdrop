...
async function main() {
    ...
    const Clothes = await hre.ethers.getContractFactory("EsotericClothing");
    const clothes = await Clothes.attach("0x2eb3495fDF23206aC02e24e5d33db404B3b4B9E0")//.connect(signer);

    const notOwners = []
    for (let i = 0; i < unminted.length; i += step) {
        await Promise.all(unminted.slice(i, i + step).map(async a=>{
            const b = await clothes.balanceOf(a.toLowerCase())
            if(b.toNumber() === 0){
                notOwners.push(a)
            }
        }))
